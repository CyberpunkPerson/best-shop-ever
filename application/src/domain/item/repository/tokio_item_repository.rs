...
impl ItemRepository for TokioItemRepository {
    fn find_all(&self) -> Vec<Item> {
        let mut client = Client::connect("host=localhost user=postgres password=postgres dbname=postgres port=5432", NoTls).unwrap();
        let rows = client.query("select * from best_shop_ever.item", &[]).unwrap();
